...
from config import BASE_DIR, REDIS_URL, TELEGRAM_BOT_TOKEN, MESSAGES, MARKET_MOVERS_CHAT_ID
...
# Ключевые фразы уведомлений «Маркет-муверы» (рост цены и высокий объём и т.п.)
_MARKET_MOVER_KEYWORDS = (
    "торгуют",
    "Легкий рост",
    "высокий объем",
    "высокий объём",
...
def _is_market_mover_text(text: str | None) -> bool:
    if not text or not text.strip():
        return False
    t = text.strip()
    # Должны быть и «торгуют», и признак роста/объёма
    if "торгуют" not in t:
        return False
    return any(k in t for k in ("Легкий рост", "высокий объем", "высокий объём", "Рост цены", "Маркет-муверы"))


def _filter_market_mover(message: Message) -> bool:
    """Фильтр: только если включён канал маркет-муверов и текст похож на уведомление маркет-муверов."""
    if not MARKET_MOVERS_CHAT_ID:
        return False
    text = (message.text or message.caption or "").strip()
    return _is_market_mover_text(text)


async def _forward_to_market_movers_channel(message: Message, bot: Bot) -> None:
    """Пересылает текст сообщения в канал маркет-муверов."""
    text = (message.text or message.caption or "").strip()
    if not text:
        return
    try:
        await bot.send_message(MARKET_MOVERS_CHAT_ID, text)
        logger.info("Market mover forwarded to %s", MARKET_MOVERS_CHAT_ID)
    except Exception as e:
        logger.warning("Market mover forward failed: %s", e)
...
def _get_welcome_text() -> str:
    ...
    return cfg.get("text") or (
        "👋 Добро пожаловать!\n\n"
        "📊 4H: PRE-ALERT (RSI 85+), сигнал ≥ 90. Redis. Маркет-муверы: «Рост цены и высокий объём».\n\n"
