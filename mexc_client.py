...
async def run_ws_ticker_stream(
    ...
    *,
    ...
    """
    sub.tickers: тот же stall + backoff, что и kline.
    """
    deadline = float(stall_timeout_sec if stall_timeout_sec is not None else WS_STALL_TIMEOUT_SEC)
    backoffs = list(backoff_delays) if backoff_delays else list(WS_RECONNECT_BACKOFF_SEC)
    if not backoffs:
    ...
    while True:
                ...
                async with session.ws_connect(MEXC_WS_URL) as ws:
                    ...
                    while True:
                        try:
                            ...
                            logger.warning("WebSocket tickers: нет данных %.0fs → reconnect", deadline)
                            break
                        if msg.type == aiohttp.WSMsgType.CLOSE:
                            break
                        if msg.type == aiohttp.WSMsgType.ERROR:
        ...
        attempt += 1
        delay = backoffs[min(attempt - 1, len(backoffs) - 1)]
        logger.info("WebSocket tickers reconnect через %.0fs (попытка %d)", delay, attempt)
        await asyncio.sleep(delay)
